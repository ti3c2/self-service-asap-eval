...
Environment overrides:
  ...
  JINA_EMBEDDINGS_MODEL       Default: jinaai/jina-embeddings-v3.
  JINA_CUDA_VISIBLE_DEVICES   Default: 1.
  JINA_PORT                   Default: 3300.
  JINA_GPU_MEMORY_UTILIZATION Default: 0.05.
EOF
}
...
TEST_MODEL_READY_TIMEOUT="${TEST_MODEL_READY_TIMEOUT:-900}"
...
JINA_EMBEDDINGS_MODEL="${JINA_EMBEDDINGS_MODEL:-jinaai/jina-embeddings-v3}"
JINA_CUDA_VISIBLE_DEVICES="${JINA_CUDA_VISIBLE_DEVICES:-1}"
JINA_PORT="${JINA_PORT:-3300}"
JINA_GPU_MEMORY_UTILIZATION="${JINA_GPU_MEMORY_UTILIZATION:-0.05}"
...
start_server jina-embeddings "$JINA_PORT" \
  env CUDA_VISIBLE_DEVICES="$JINA_CUDA_VISIBLE_DEVICES" \
  VLLM_USE_FLASHINFER_SAMPLER="$VLLM_USE_FLASHINFER_SAMPLER" \
  "${vllm_cmd[@]}" serve "$JINA_EMBEDDINGS_MODEL" \
  --port "$JINA_PORT" \
  --trust-remote-code \
  --gpu-memory-utilization "$JINA_GPU_MEMORY_UTILIZATION"
